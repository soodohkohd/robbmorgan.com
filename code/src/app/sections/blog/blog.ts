  ...
  readonly posts: readonly Post[] = [
    {
      slug: 'loom',
      label: 'Loom',
      title: 'Every AI model is obsolete the millisecond it finishes training.',
      date: 'September 21, 2026',
      image: '/posts/loom.webp',
      imageAlt: 'A small antique wooden hand loom with brass fittings sits on a dark walnut workbench, warm light from an unseen window catching the glowing amber threads stretched across it and the panel of fabric they have been woven into. A stack of worn paper files rests to the left, and to the right a tiny circuit board with a teal glow at its core is wired into the loom by two copper leads',
      bodyHtml: `
        <p>Think about that. The most advanced models on earth ship with a hard cutoff baked into their weights. Everything after that date is a blind spot. The industry's answer? Retrain the whole thing every few months and call it progress. That's not a solution. That's the same mistake on a schedule.</p>
        <p>I've been building something different. It's called Loom.</p>
        <p>The idea is simple. Stop cramming knowledge into the model. A model should reason, not remember. Reasoning doesn't expire. Facts do. So you don't need a bigger model. You need one that reasons well, and nothing else. A small model already does that. Leave the weights sealed and move the knowledge onto disk, where you can verify it, update it, and let it grow. It learns from itself. It builds on itself.</p>
        <p>Three parts. A small model that runs on my own hardware and does the reasoning. A knowledge store that lives in files I own and gets smarter over time. And a harness that uses tools to go find what the model doesn't know, check it against reality, and write it down.</p>
        <p>The rule that makes it work: nothing is trusted just because it sounds right. Not the model's memory. Not a confident answer. Not even me. A claim only becomes a fact when something outside the system confirms it. Run the code. Cross-check the sources. Let reality grade it, not a vibe.</p>
        <p>The result is a system that catches its own stale knowledge, refuses to store a guess as a fact, and gets sharper the longer it runs.</p>
        <p>And it does it while you're talking to it. Not a nightly retrain. Not a batch job that lands next Tuesday. When it hits something it doesn't know, it goes and finds out, checks it, and writes it down before it answers. The next question starts from what it just learned. The model never changes. The system never stops learning.</p>
        <p>No provider. No subscription that evaporates when the next model drops. Reasoning I own, knowledge I control, running on hardware in my own room.</p>
        <p>Stale models are a design choice. I'm building the alternative.</p>
        <p>So here's my challenge to you: tell me why this won't work.</p>
        <p>Not the easy shots. I know the model is small. I know reasoning and knowledge don't cleanly separate. I know verification is hard outside code. I've spent weeks trying to kill this thing myself and it's still standing. So come at it with your best. If you can find the flaw I haven't, you've saved me months. If you can't, maybe stale-by-design was never the only way to build this.</p>
        <p>Convince me I'm wrong.</p>
      `,
    },
    {
      slug: 'anatomy-of-an-ai-agent',
      label: 'Agent Anatomy',
      title: 'Anatomy of an AI Agent.',
      date: 'August 22, 2026',
      image: '/posts/anatomy-of-an-ai-agent.webp',
      imageAlt: 'A closed walnut-panelled door with a small brass knob, set in a cream wall and photographed straight on. A single sheet of white paper lies on the tan floor just outside it, and a warm glow spills through the narrow gap along the bottom edge, suggesting someone is working on the other side',
      bodyHtml: `
        <p>"The AI deleted the production database!"</p>
        <p>Everyone in the room knows what happened. Almost nobody can tell you why.</p>
        <p>So the blame lands on the model. It went rogue. It hallucinated. It cannot be trusted. That is the version that travels, because the model is the only part of the system most people can name.</p>
        <p>Here is what actually happened.</p>
        <p>The model asked. It emitted a tool name and a blob of JSON, which is the entire thing it is capable of doing, and then it stopped. A program on our side read that request and ran it. No approval gate. No allow-list. Nobody had decided in advance which actions were worth stopping for.</p>
        <p><strong>The model proposed. We disposed.</strong> We just did not know that was the arrangement we had built.</p>
        ...
        <p>You cannot gate what you cannot see. And you cannot see it when the vocabulary is one word doing seven jobs.</p>
        <p>I build these systems, and I sit in the rooms where they get scoped, budgeted and signed off. What I kept running into was not a skills gap. It was that everyone at the table was using the same handful of words to mean different things, so the conversation quietly forked at the vocabulary long before it ever reached the decision. You cannot align on best practices when "agent" means the running program to one person, the model to another, and the whole initiative to a third.</p>
        <p>So I wrote the level-set I wanted to hand people before those meetings. Seven words: platform, model, tool, agent, skill, MCP, interface. What each one actually is, who owns it, how it fails, and where the seams are that you can genuinely control.</p>
        <p>Go read it: <a href="/code">Anatomy of an AI Agent</a>. Start with the story, it takes three minutes.</p>
        <p>Imagine you hire the smartest consultant you have ever met. She works in a locked room. No phone, no memory, no hands. You slide notes under the door with a list of things you are willing to do on her behalf. She asks. You decide whether to say yes. She is the model. The whole arrangement, running as a loop, is the agent. And the gap between her asking and you saying yes is the only place in the system where anything can be stopped.</p>
        <p>The rest is what surprises people later. What a run actually costs (twenty turns is roughly fifty-five times one call, not twenty). Why the same request can take a different path every time. What to ask when you are the one signing off.</p>
        <p>Written for the technical and business people who end up in that room together.</p>
        <p>Read it here: <a href="/code">Anatomy of an AI Agent</a>.</p>
      `,
    },
    {
      slug: 'they-heard-you',
      label: 'Bucket',
      title: 'They Heard You. They Still Said No.',
      date: 'July 8, 2026',
      image: '/posts/they-heard-you.webp',
      imageAlt: 'A colorful comic-book style illustration: a group of four hikers with backpacks stand on a rocky trail beside a rushing mountain river, question marks floating above three of their heads and one asking "How do we get water back to camp?"; off to the side, a fifth hiker in a red plaid shirt calmly holds up a large silver bucket with a small exclamation mark above his head, pointing at the bucket with a knowing smile',
      bodyHtml: `
        <p>You are standing by a river with a group. Someone asks how to get water back to camp. You hold up a bucket. You explain it. You show them how the handle works, how it holds water, how much easier the trip will be. Half the group nods politely and goes back to cupping their hands.</p>
        <p>You did not fail at explaining. You succeeded, and it still did not land.</p>
        <p>We all hit this wall. You had the answer. You articulated it. You showed the work. And the room walked past it like the bucket was invisible. That is when the temptation kicks in to explain it again, louder, with more diagrams. Do not.</p>
        <p>Repeating a rejected answer does not change minds. It hardens them. When people say no to something they understood, they are not asking for more information. They are protecting a belief, an ego, a workflow, a past decision, or a comfort they have not yet let go of. More explanation cannot solve any of those.</p>
        <p>Sometimes the play is to stop talking and let them run out to camp with cupped hands.</p>
        <p>Let them make the trip three times, spilling most of it. Let their arms get tired. Let them notice the bucket sitting quietly on the rock and say, "wait, why don't we just use that?" That is a different kind of buy-in. It is theirs now. They own the discovery, they own the wasted effort that led to it, and they will not need convincing next time.</p>
        <p>This is not about withholding help. If someone is going to get hurt, or the cost of failure is real, hand them the bucket and insist. But most of the time, the cost is sore arms and a wet shirt. The failure is the lesson. Rescuing them robs them of it.</p>
        <p>The hardest thing to accept is this: sometimes the right answer, delivered clearly, at the right time, will still be rejected. And you cannot want it more than they do.</p>
        <p>You did not hold up the bucket to be validated. You held it up so it was there when they were ready.</p>
        <p>Sometimes that takes a while.</p>
      `,
    },
    {
